...
- `.github/instructions/` — GitHub Copilot instruction files; **not for ZCode agents** (ignore unless working on Copilot behavior itself)
...
Verify changes by opening the app in a browser and running a test. `node --check src/script.js` (and `sw.js`) is a quick syntax sanity check. Note the service worker caches `src/` assets cache-first: after changing a cached file, verify on a fresh port (e.g. `python3 -m http.server 8001`) or a `?query` URL, otherwise the browser keeps serving the stale copy.
...
- **Latency color thresholds** (≤50 ms green, ≤150 ms yellow, >150 ms red) are duplicated in `renderResultItem()` and `renderBars()`, and the same numbers are hardcoded in the filter chip labels in `index.html` — update all three places when changing them.
- **Service worker**: `sw.js` lives at the repo root (its scope must cover `index.html`), has a hardcoded `urlsToCache` list and `CACHE_NAME` (`doh-tester-v5`), and is registered at the bottom of `src/script.js`. When adding static assets — or after changing any cached asset — add them to the cache list and bump the cache name, otherwise returning visitors keep getting the stale cached copy. The worker does **not** call `skipWaiting()` on install; instead the page shows an "Update available" toast and sends a `skip-waiting` message when the user confirms the reload (so an update never swaps in under a running test). `data/` files use network-first caching so the weekly auto-updated server list isn't hidden behind stale cache.
...
- **Result row state**: each `<li>` in `#resultsList` carries `data-speed` (`fast`/`medium`/`slow`/`error`/`pending`) and, while untested, `data-pending`. The filter chips + URL search (`applyFilter()`) and `finalizePendingTests()` rely on these attributes — don't detect pending rows by their text content. `updateSummary()` recomputes the stat cards from `testResults`; call it wherever results change.
- **Buttons with icons**: `init()` and the Settings/About toggles rewrite button `innerHTML` including the Material Symbols `<span>` — never use `textContent` on those buttons or the icon gets wiped. Install (`beforeinstallprompt`, Chromium only), Share (`navigator.share`) and the offline badge are feature-detected and stay hidden when unsupported.
...
- CSS: use the existing custom properties (`--bg-body`, `--primary`, `--bar-bg`, etc.) rather than hardcoded colors; dark mode is a mix of `:root[data-theme='dark']` (user choice) and `@media (prefers-color-scheme: dark)` on `:root:not([data-theme])` (OS default) — see Theming above. `color-mix()` is used with a solid fallback declaration on the preceding line — keep both lines when editing.
